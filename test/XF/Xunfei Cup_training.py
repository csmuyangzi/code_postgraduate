...
if os.path.exists('model.config'): 
    ...
    class data_for_word2vec: #用迭代器将语料整合起来
        def __iter__(self):
            for x in train_x:
                yield x[0]
                yield x[1]
...
for e in range(epochs):
    ...
    for x in train_data:
        if batch % 10 == 0:
            ...
            saver.save(sess, './xf/xf_%s.ckpt'%e) #每个epoch保存一次
        ...
        if batch % 100 == 0:
            ...
            if highest_acc <= acc:
                highest_acc = acc
                saver.save(sess, './xf/xf_highest.ckpt') #历史最好也保存一次
        batch += 1
        count += len(x[0])